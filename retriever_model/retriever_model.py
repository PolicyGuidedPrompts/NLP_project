...
class RetrieverModel(ABC):
    ...
    def model_path(self):
        if hasattr(self, "repository"):
            return self.repository + "/" + self.model_name
...
class SBertRetriever(RetrieverModel):
    ...
    def __init__(self, config, dataset):
        ...
        ), "retriever_top_k is required when using sbert model"
    ...
    def encode(self, encoder_input):
        encoding = (
            self.model.encode(encoder_input, convert_to_tensor=True)
            .detach()
            .cpu()
            .numpy()
        )
    ...
    def retrieve(self, encoding, mode):
        ...
        if mode == "train":
            hits = util.semantic_search(
                query_embeddings=encoding_tensor,
...
class Encoder(RetrieverModel):
    def __init__(self, config, dataset):
        assert (
            not config.retriever_top_k
        ), f"{self.model_name} encoder does not support retriever_top_k"
        super().__init__(self.model_name, config)
