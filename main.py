...
ALLOWED_LLMS = (
    AVAILABLE_LLM_MODELS.keys()
)  # 'gpt2','gpt3.5','llama-2-7b','flan-t5-base','flan-t5-small'
ALLOWED_ENCODERS = (
    AVAILABLE_ENCODERS.keys()
)  # 'bert-base-uncased','bge-large-en','gte-large'
...
# Encoder
parser.add_argument(
    "--encoder_model", type=str, required=True, choices=ALLOWED_ENCODERS
)
parser.add_argument(
...
parser.add_argument(
    "--num_episodes_per_batch", type=int, default=10
...
parser.add_argument(
    "--initial_temperature", type=float, default=400
)  # For both linear and exponential
parser.add_argument(
    "--end_temperature", type=float, default=1
...
if __name__ == "__main__":
    ...
    encoder = EncoderFactory.create_encoder(
        model_name=namespace.encoder_model, config=config
    )
